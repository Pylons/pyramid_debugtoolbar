...
- If the configuration setting ``debugtoolbar.intercept_exc`` is ``true``,
  when an exception is raised by your application, an interactive debugging
  console will appear.

- If the configuration setting ``debugtoolbar.intercept_redirects`` is
  ``true``, when a redirect is issued by your application, you will be given
  the choice to examine the state of the redirect page before proceeding to
