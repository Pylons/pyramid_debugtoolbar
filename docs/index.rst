...
``debugtoolbar.show_on_exc_only``
  ...
  if the website uses the same area. And especially in large projects the html
  code generated by the toolbar slows down the page rendering significantly.
...
.. note::
   ...
   static route and ``/_debug_toolbar/static/*`` contents will still be
   accessible.
